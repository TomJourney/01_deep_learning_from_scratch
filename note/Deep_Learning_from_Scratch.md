...
y=h(a)
$$
则输入信号总和为a，函数h()把a转为输出y，则h()就是激活函数； 
...
1）仿射变换定义：指在几何中，对一个向量空间进行一次线性变换并接上一个平移，变换为另一个向量空间。分别对应神经网络的加权运算与加偏置项运算； 

【例】仿射变换：一个向量$X$平移$B$，与旋转缩放$W$的仿射变换为：
$$
Y=X \cdot W+B
$$

2）Affine仿射变换正向传播的计算图（注意变量是矩阵）如下：

![](./ch05_p146b.png)

【补充】 形状(2,)可以简单理解为1行2列的矩阵，或者长度为2的一维数组；

4）Affine仿射变换反向传播的计算图如下：
...
1） 5.6.1介绍的X的形状是(2,) ，以单个数据为对象；下面考虑以N个数据一起正向传播的情况，即批版本的Affine层；

2）批版本的Affine层定义：指N个数据（数据项）的整体经过线性变换，再进行平移得到的另一个向量空间；

3）批版本的Affine层的计算图，如图5-27所示。

![](./ch05_p148.png)

【补充】

- 与图5-24不同的是，图5-24中$X$的形状是(2,)，转换为2维数组也就是(1,2)；
- 而图5-27中，$X$的形状是(N,2)
